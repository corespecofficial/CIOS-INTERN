...
export function AdminDashboard(props: Props) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      >
        <img src={MASCOT} alt="Mascot" style={{ width: 56, height: 56, borderRadius: "50%", objectFit: "cover" }} />
        <div style={{ flex: 1 }}>
          <span
            style={{
          ...
          <p style={{ fontSize: 14, color: "#8892A4", margin: "4px 0 0 0" }}>
            {isPreview
              ? "Preview mode — visit /admin for live data."
              : `${stats.total} total users · ${stats.activeNow} active in last 24h`}
          </p>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16 }}>
        {[
          { value: stats.interns.toString(), label: "Interns + Team Leads", color: "#1E88E5" },
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 12 }}>
          {adminActions.map((a) => {
            ...
            const s: React.CSSProperties = {
              background: "#0A0E1A", borderRadius: 14, padding: 16,
              textAlign: "center", border: "1px solid rgba(255,255,255,0.07)",
              cursor: "pointer", display: "flex", flexDirection: "column",
              alignItems: "center", gap: 6, textDecoration: "none", color: "inherit",
            };
            return a.href
      ...
      <div style={{ background: "#111827", borderRadius: 16, padding: 24, border: "1px solid rgba(255,255,255,0.07)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <h3 style={{ fontSize: 16, fontWeight: 600, color: "#E8EDF5", margin: 0 }}>
            User Management
          </h3>
          <Link href="/super-admin/users" style={{ fontSize: 12, color: "#1E88E5", textDecoration: "none", fontWeight: 600 }}>
            View All →
          </Link>
        </div>
        {users.length === 0 ? (
          <p style={{ fontSize: 13, color: "#8892A4" }}>No users yet. Invite some via Manage Users.</p>
        ) : (
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                {["User", "Role", "Status", "Performance", "XP"].map((h) => (
                  <th key={h} style={{ textAlign: "left", padding: "10px 12px", fontSize: 12, color: "#8892A4", fontWeight: 500 }}>
                    {h}
                  </th>
                ))}
              </tr>
              ...
              {users.map((u) => {
                ...
                return (
                  <tr key={u.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                    ...
                    <td style={{ padding: "12px 12px", fontSize: 12, color: "#8892A4", textTransform: "capitalize" }}>
                      {u.role.replace("_", " ")}
                    </td>
                    ...
                    <td style={{ padding: "12px 12px", fontSize: 14, color: "#E8EDF5", fontFamily: "monospace" }}>
                      {u.xp.toLocaleString()}
                    </td>
                  </tr>
                );
      ...
      <div style={{ background: "#111827", borderRadius: 16, padding: 24, border: "1px solid rgba(255,255,255,0.07)" }}>
        ...
        {audits.length === 0 ? (
          <p style={{ fontSize: 13, color: "#8892A4" }}>
            No audit log entries yet. Sensitive actions will be recorded here once we wire audit logging.
          </p>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {audits.map((item) => (
              <div key={item.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "8px 0" }}>
                <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#1E88E5", flexShrink: 0 }} />
                <span style={{ flex: 1, fontSize: 14, color: "#E8EDF5" }}>
                  {item.actor_name ? `${item.actor_name} ` : ""}<b>{item.action}</b> <span style={{ color: "#8892A4" }}>{item.entity_type}</span>
                </span>
                <span style={{ fontSize: 12, color: "#8892A4", flexShrink: 0 }}>{timeAgo(item.created_at)}</span>
              </div>
            ))}
          </div>
        )}
