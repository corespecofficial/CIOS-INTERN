...
export default async function GamificationPage() {
  ...
  const user = snap.user || { id: me.id, name: me.name, avatar_url: me.avatar_url, role: me.role, xp: 0, level: 1, streak: 0, best_streak: 0, reputation: 0, coins: 0 };
  const progress = levelProgress(user.xp || 0);
  const rank = rankFromLevel(user.level || 1);

  return (
    <GamificationHub
      user={{ id: user.id, name: user.name, avatarUrl: user.avatar_url, role: user.role, xp: user.xp, level: user.level, streak: user.streak, bestStreak: user.best_streak || 0, reputation: user.reputation, coins: user.coins || 0 }}
      progress={progress}
      rank={rank}
