...
const KINDS = [
  { key: "all", label: "All", color: "var(--text-tertiary, #8892A4)" },
  ...
  { key: "other", label: "Other", color: "var(--text-tertiary, #8892A4)" },
];
    ...
    <div style={{ maxWidth: 1100, margin: "0 auto", fontFamily: "'Nunito', sans-serif" }}>
      ...
      <div style={{ background: "linear-gradient(135deg, rgba(255,193,7,0.12), rgba(239,83,80,0.06))", border: "1px solid rgba(255,193,7,0.2)", borderRadius: 16, padding: 22, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12, marginBottom: 16 }}>
        <div>
          ...
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "var(--text-primary, #E8EDF5)", margin: 0 }}>📁 My documents</h1>
          <p style={{ fontSize: 13, color: "var(--text-tertiary, #8892A4)", margin: "2px 0 0 0" }}>Certificates · CVs · reports · contracts — all in one place</p>
        </div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          <a href="/documents/app/cv" style={btnGhost}>📄 Build CV with AI</a>
          <a href="/documents/app" style={btnGhost}>🛠️ All tools</a>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 10, marginBottom: 16 }}>
        {KINDS.slice(1, 7).map((k) => (
          <div key={k.key} onClick={() => setKind(k.key)} style={{ background: "var(--bg-secondary, #111827)", border: `1px solid ${k.color}33`, borderRadius: 12, padding: 14, cursor: "pointer" }}>
            <div style={{ fontSize: 10, color: "var(--text-tertiary, #8892A4)", letterSpacing: 0.5, textTransform: "uppercase" }}>{k.label}</div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 4, background: "var(--bg-secondary, #111827)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: 4, overflowX: "auto" }}>
          {KINDS.map((k) => (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 12 }}>
        {filtered.length === 0 && (
          <div style={{ gridColumn: "1/-1", padding: 40, textAlign: "center", color: "var(--text-tertiary, #8892A4)", background: "var(--bg-secondary, #111827)", border: "1px dashed rgba(255,255,255,0.1)", borderRadius: 14 }}>
        ...
        {filtered.map((d) => {
          ...
          return (
            <div key={d.id} style={{ background: "var(--bg-secondary, #111827)", border: `1px solid ${meta.color}22`, borderRadius: 12, overflow: "hidden", display: "flex", flexDirection: "column" }}>
              ...
              <div style={{ padding: 12 }}>
                {renaming?.id === d.id ? (
                  ...
                  <div onClick={() => setRenaming({ id: d.id, name: d.name })} style={{ fontSize: 13, fontWeight: 700, color: "var(--text-primary, #E8EDF5)", marginBottom: 4, cursor: "text", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{d.name}</div>
                )}
                <div style={{ fontSize: 10, color: "var(--text-tertiary, #8892A4)", display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                ...
                <div style={{ display: "flex", gap: 4 }}>
                  ...
                  <a href={d.url === "/api/cv" ? "/api/cv?download=1" : d.url} target="_blank" rel="noopener noreferrer" download={d.name} style={{ padding: "6px 10px", background: "transparent", color: "var(--text-primary, #E8EDF5)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 6, fontSize: 11, fontWeight: 700, textDecoration: "none" }}>↓</a>
      ...
      {uploading && <div style={{ position: "fixed", bottom: 20, right: 20, background: "var(--bg-secondary, #111827)", border: "1px solid rgba(30,136,229,0.3)", borderRadius: 10, padding: "10px 16px", color: "#1E88E5", fontSize: 12, fontWeight: 700, zIndex: 100 }}>⬆ Uploading…</div>}
  ...
  return (
    <div onClick={(e) => e.target === e.currentTarget && onClose()} style={{
      ...
      <div style={{
        ...
        background: "var(--bg-secondary, #111827)", border: "1px solid rgba(255,255,255,0.1)",
        ...
        <div style={{ padding: "12px 18px", borderBottom: "1px solid rgba(255,255,255,0.07)", display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 14, fontWeight: 800, color: "var(--text-primary, #E8EDF5)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{doc.name}</div>
            <div style={{ fontSize: 11, color: "var(--text-tertiary, #8892A4)", marginTop: 2 }}>{doc.mime || "Document"}</div>
          </div>
          ...
          <button onClick={onClose} aria-label="Close" style={{ width: 36, height: 36, borderRadius: 8, background: "transparent", color: "var(--text-tertiary, #8892A4)", border: "1px solid rgba(255,255,255,0.08)", cursor: "pointer", fontSize: 16, flexShrink: 0 }}>✕</button>
        </div>

        {/* Body */}
        <div style={{ flex: 1, background: "var(--bg-primary, #0A0E1A)", minHeight: 300, overflow: "hidden" }}>
          {isPdf ? (
            ...
            <div style={{ padding: 40, textAlign: "center", color: "var(--text-tertiary, #8892A4)" }}>
