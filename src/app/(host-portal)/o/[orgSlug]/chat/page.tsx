...
export default async function HostChatPage({ params }: { params: Promise<{ orgSlug: string }> }) {
  ...
  const [rooms, directory, statuses] = await Promise.all([
    listMyRooms(ctx.org.id), listDirectoryUsers(ctx.org.id), listActiveStatuses(ctx.org.id),
  ]);
