...
// 5s revalidate is the FALLBACK path for when Ably isn't configured
// (no NEXT_PUBLIC_ABLY_API_KEY). When Ably IS configured, ChatLive
// receives messages instantly via the realtime subscription and this
// polling is just a safety net for missed publishes.
...
export default async function ChannelPage({ params }: { params: Promise<{ orgSlug: string; channelId: string }> }) {
  ...
  if (!channel) notFound();
  // Reversed because we sort DESC for index efficiency, then flip in-memory.
  ...
  return (
    <div style={{ display: "flex", gap: 16, height: "calc(100dvh - 64px)" }}>
      <aside style={{ width: 200, background: "#111827", border: "1px solid #1F2937", borderRadius: 10, padding: 12, overflowY: "auto" }}>
        <div style={{ fontSize: 10, color: "#5A6478", textTransform: "uppercase", letterSpacing: 0.5, padding: "4px 6px", marginBottom: 4 }}>
          Channels
        </div>
        {channels.map((c) => {
      ...
      <section style={{ flex: 1, display: "flex", flexDirection: "column", background: "#111827", border: "1px solid #1F2937", borderRadius: 10, overflow: "hidden" }}>
        <header style={{ padding: "14px 18px", borderBottom: "1px solid #1F2937" }}>
          ...
          <div style={{ fontSize: 11, color: "#5A6478" }}>
            {messages.length} recent messages · live via Ably (5s polling fallback)
          </div>
        </header>
