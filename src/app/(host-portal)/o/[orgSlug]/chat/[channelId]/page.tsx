...
// Auto-refresh every 5s — cheap polling beats wiring Ably for Phase 4.
// Phase 5 swaps this for Ably realtime.
    ...
    <div style={{ display: "flex", gap: 16, height: "calc(100dvh - 64px)" }}>
      ...
      <section style={{ flex: 1, display: "flex", flexDirection: "column", background: "#111827", border: "1px solid #1F2937", borderRadius: 10, overflow: "hidden" }}>
        <header style={{ padding: "14px 18px", borderBottom: "1px solid #1F2937" }}>
          ...
          <div style={{ fontSize: 11, color: "#5A6478" }}>{messages.length} recent messages · auto-refreshes every 5s</div>
        </header>

        <div style={{ flex: 1, overflowY: "auto", padding: "16px 18px", display: "flex", flexDirection: "column", gap: 10 }}>
          {messages.length === 0 ? (
            <div style={{ textAlign: "center", color: "#5A6478", fontSize: 13, marginTop: 40 }}>No messages yet — start the conversation.</div>
          ) : (
            messages.map((m) => (
