  ...
  for (const u of users as Array<{ id: string; name: string | null; email: string | null; xp: number; streak: number; notification_prefs: { digestFrequency?: string } | null }>) {
    ...
    try {
      ...
      const html = wrapEmail(`
        ...
        <a href="${process.env.NEXT_PUBLIC_APP_URL || ""}/dashboard" style="display:inline-block;margin-top:14px;padding:12px 28px;background:linear-gradient(135deg,#1E88E5,#1565C0);color:#fff;border-radius:10px;font-weight:700;text-decoration:none;">Open dashboard →</a>
